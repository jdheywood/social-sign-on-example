...
module.exports = {
   ...
   */
  getDayOfWeek() {
    let weekDayAsNumber = moment().tz('Europe/London').isoWeekday();
   ...
   */
  getTimeOfDay() {
    let d = moment().tz('Europe/London');
    let currentHour = d.hour();
    if (currentHour >= appConfig.app.morningStart && currentHour < appConfig.app.eveningStart) {
      return 'Morning';
    } else {
      return 'Evening';
    }
   ...
   */
  getPreviousDayOfTheWeek(currentDayOfTheWeek) {
    let previousDayOfTheWeek = null;
   ...
   */
  getMaxWeekNumberByYear(year) {
    let maxWeekNumber = 52;
    if (year === 2015) {
      maxWeekNumber = 53;
    } else if (year === 2016) {
      maxWeekNumber = 52;
  ...
  getCurrentPeriod() {
    ...
    let currentPeriod = {
      ...
      weekNumber: moment().tz('Europe/London').isoWeek(),
      year: moment().tz('Europe/London').isoWeekYear()
    };

    let currentHour = moment().tz('Europe/London').hour();

    if (currentPeriod.timeOfDay === 'Evening' && currentHour < 4) {
      let previousDay = self.getPreviousDayOfTheWeek(currentPeriod.dayOfWeek);
